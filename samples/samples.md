# List of Linux /proc file samples

## Sources
- [The /proc Filesystem - The Linux Kernel](https://docs.kernel.org/filesystems/proc.html) - Official documentation - 6.18.0 (2025, 2009)
- [RedHat Enterprise Linux 4 documentation - 5. The proc File System](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/4/html/reference_guide/ch-proc) (circa 2005)
----
## Folders - Subfolders

## /proc/[PID]
- PID is the process indentification number
- [PID/maps-kernel-doc.txt](PID/maps-kernel-doc.txt)
- [PID/numa_maps-kernel-doc.txt](PID/numa_maps-kernel-doc.txt)
- [PID/smaps-kernel-doc.txt](PID/smaps-kernel-doc.txt)

## /proc/irq
- [irq/irq-kernel-doc.txt](irq/irq-kernel-doc.txt)

## /proc/net
- [net/dev-kernel-doc.txt](net/dev-kernel-doc.txt)

## /proc/self
- [self/status-kernel-doc.txt](self/status-kernel-doc.txt)

----
## Files

## /proc/allocinfo
- [allocinfo/allocinfo-kernel-doc.txt](allocinfo/allocinfo-kernel-doc.txt)

## /proc/apm
- [apm/apm-rhel-4-doc.txt](apm/apm-rhel-4-doc.txt)

## /proc/buddyinfo
- [buddyinfo/buddyinfo-rhel-4-doc.txt](buddyinfo/buddyinfo-rhel-4-doc.txt)
- [buddyinfo/buddyinfo-kernel-doc.txt](buddyinfo/buddyinfo-kernel-doc.txt)

## /proc/cmdline
- [cmdline-rhel-4-doc.txt](cmdline-rhel-4-doc.txt)

## /proc/cpuinfo
- [cpuinfo/cpuinfo-rhel-4-doc.txt](cpuinfo/cpuinfo-rhel-4-doc.txt)

## /proc/crypto
- [crypto/crypto-rhel-4-doc.txt](crypto/crypto-rhel-4-doc.txt)

## /proc/devices
- [devices/devices-rhel-4-doc.txt](devices/devices-rhel-4-doc.txt)

## /proc/dma
- [dma/dma-rhel-4-doc.txt](dma/dma-rhel-4-doc.txt)

## /proc/execdomains
- [execdomains/execdomains-rhel-4-doc.txt](execdomains/execdomains-rhel-4-doc.txt)

## /proc/fb
- [fb/fb-rhel-4-doc.txt](fb/fb-rhel-4-doc.txt)

## /proc/filesystems
- [filesystems/filesystems-rhel-4-doc.txt](filesystems/filesystems-rhel-4-doc.txt)

## /proc/interrupts
- **Single processor**:
  - [interrupts/interrupts-rhel-4-doc-1.txt](interrupts/interrupts-rhel-4-doc-1.txt)
  - [interrupts/interrupts-kernel-doc-1.txt](interrupts/interrupts-kernel-doc-1.txt)
- **Multiple processor**:
  - [interrupts/interrupts-kernel-doc-2.txt](interrupts/interrupts-kernel-doc-2.txt)
  - [interrupts/interrupts-rhel-4-doc-2.txt](interrupts/interrupts-rhel-4-doc-2.txt)

## /proc/iomem
- [iomem/iomem-rhel-4-doc.txt](iomem/iomem-rhel-4-doc.txt)

## /proc/ioports
- [ioports/ioports-rhel-4-doc.txt](ioports/ioports-rhel-4-doc.txt)

## /proc/kcore
- placeholder

## /proc/kmsg
- placeholder

## /proc/loadavg
- [loadavg/loadavg-rhel-4-doc.txt](loadavg/loadavg-rhel-4-doc.txt)

## /proc/locks
- [locks/locks-rhel-4-doc.txt](locks/locks-rhel-4-doc.txt)

## /proc/mdstat
- **Without RAID configured**: [mdstat/mdstat-rhel-4-doc-1.txt](mdstat/mdstat-rhel-4-doc-1.txt)
- **With md0 RAID**: - [mdstat/mdstat-rhel-4-doc.txt](mdstat/mdstat-rhel-4-doc.txt)

## /proc/meminfo
- [meminfo/meminfo-rhel-4-doc.txt](meminfo/meminfo-rhel-4-doc.txt)
- [meminfo/meminfo-kernel-doc.txt](meminfo/meminfo-kernel-doc.txt)

## /proc/misc
- [misc/misc-rhel-4-doc.txt](misc/misc-rhel-4-doc.txt)

## /proc/modules
- [modules/modules-rhel-4-doc.txt](modules/modules-rhel-4-doc.txt)

## /proc/mounts
- [mounts/mounts-rhel-4-doc.txt](mounts/mounts-rhel-4-doc.txt)

## /proc/mtrr
- [mtrr/mtrr-rhel-4-doc.txt](mtrr/mtrr-rhel-4-doc.txt)

## /proc/pagetypeinfo
- [pagetypeinfo/pagetypeinfo-kernel-doc.txt](pagetypeinfo/pagetypeinfo-kernel-doc.txt)

## /proc/partitions
- [partitions/partitions-rhel-4-doc.txt](partitions/partitions-rhel-4-doc.txt)

## /proc/pci
- [pci/pci-rhel-4-doc.txt](pci/pci-rhel-4-doc.txt)

## /proc/scsi
- [scsi/scsi-kernel-doc.txt](scsi/scsi-kernel-doc.txt])

## /proc/softirqs
- [softirqs/softirqs-kernel-doc.txt](softirqs/softirqs-kernel-doc.txt)

## /proc/slabinfo
- [slabinfo/slabinfo-rhel-4-doc.txt](slabinfo/slabinfo-rhel-4-doc.txt)

## /proc/stat
- [stat/stat-rhel-4-doc.txt](stat/stat-rhel-4-doc.txt)

## /proc/swaps
- [swaps/swaps-rhel-4-doc.txt](swaps/swaps-rhel-4-doc.txt)

## /proc/sysrq-trigger
- placeholder

## /proc/uptime
- [uptime-rhel-4-doc.txt](uptime-rhel-4-doc.txt)

## /proc/version
- [version/version-rhel-4-doc.txt](version/version-rhel-4-doc.txt)

## /proc/vmallocinfo
- [vmallocinfo/vmallocinfo-kernel-doc.txt](vmallocinfo/vmallocinfo-kernel-doc.txt)
